...
package com.zihuatanejo.finalcamera.plugins.processing.preshot;
...
import com.zihuatanejo.finalcamera.plugins.capture.preshot.PreShot;
...
public class PreshotProcessingPlugin extends PluginProcessing implements OnTouchListener, OnClickListener {
    ...
    private void flipPhoto(boolean toLeft, float XtoVisible) {
        ...
        if (mLayoutOrientationCurrent == 90 || mLayoutOrientationCurrent == 270)
            duration_visible = Util.clamp(Math.abs(Math.round((XtoVisible * 500) / actH)), 250, 500);
        else
            duration_visible = Util.clamp(Math.abs(Math.round((XtoVisible * 500) / actW)), 250, 500);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (difX > Xprev) {
                    ...
                    scale_from = Util.clamp(scale_from, 1, 2);
                    scale_to = Util.clamp(scale_to, 1, 2);
