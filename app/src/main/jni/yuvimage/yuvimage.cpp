...
void ExtractYuvFromDirectBuffer(unsigned char *Y, unsigned char *U,
		unsigned char *V, unsigned char *yuv, jint pixelStrideY,
		jint rowStrideY, jint pixelStrideU, jint rowStrideU, jint pixelStrideV,
		jint rowStrideV, jint sx, jint sy) {
	int i, y;
...
#pragma omp parallel for schedule(guided)
	for (y = 0; y < sy; y += 2) {
		...
		for (x = 0; x < sx / 2; ++x) {
			// V
			yuv[sx * sy + (y / 2) * sx + x * 2] = V[x * pixelStrideV
					+ (y / 2) * rowStrideV];

			// U
			yuv[sx * sy + (y / 2) * sx + x * 2 + 1] = U[x * pixelStrideU
					+ (y / 2) * rowStrideU];
		}
	}
